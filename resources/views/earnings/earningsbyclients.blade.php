<x-layouts.porto title="Earnings by Clients per Month" header="Earnings by Clients per Month">
    ...
    @if(isset($earningsofclients))
      ...
      <h3>Total earnings generated by <strong>{{$earningsperclient['name']}}</strong> per month: <strong><font color="green">{{$earningsperclient['sum']}} USD</font>,<font color="blue"> {{($earningsperclient['sum']/$totalsum)*100}} percent</font></strong></h3><br>
      @endforeach
    @else
